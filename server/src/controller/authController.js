...
import { successResponseWithData, successResponse, validationError } from '../utils/apiResponse.js';
...
const verifyToken = tryCatchAsync(async (req, res) => {
  ...
  if (!resetToken) {
    ...
    return successResponseWithData(res, 'Token verified successfully', { user: result });
  }
  const result = await authService.verifyToken(token);
  return successResponseWithData(res, 'Token verified successfully', { user: result });
})
